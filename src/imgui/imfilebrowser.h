...
{
    PushID(this);
    FileBrowser::ScopeGuard exitThis([this]
    {
        openFlag_ = false;
        closeFlag_ = false;
        PopID();
    ...
    if(newDirNameBuf_)
    {
        ...
        {
          FileBrowser::ScopeGuard endNewDirPopup([] { EndPopup(); });
            ...
            {
              FileBrowser::ScopeGuard closeNewDirPopup([] { CloseCurrentPopup(); });
        ...
        reserveHeight += GetFrameHeightWithSpacing();
    {
        ...
        FileBrowser::ScopeGuard endChild([] { EndChild(); });
    ...
    {
        PushID(this);
        FileBrowser::ScopeGuard popTextID([] { PopID(); });
    ...
    if(!typeFilters_.empty())
    {
        ...
        {
          FileBrowser::ScopeGuard guard([&] { EndCombo(); });
